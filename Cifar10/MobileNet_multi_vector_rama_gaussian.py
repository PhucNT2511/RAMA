...
class MobileNet(nn.Module):
    """
    Modified MobileNet-V3 architecture with Gaussian RAMA layers at multiple positions.
    ...
    def __init__(self, num_classes=10, use_rama=False, rama_config=None):
        ...
        self.features_2 = nn.Sequential(
            self.backbone.classifier[0],   # Linear(576 → 1024)
            self.backbone.classifier[1],   # Hardswish()
            self.backbone.classifier[2],   # Dropout
        )

        self.features_3 = nn.Linear(self.feature_dim, num_classes)
    ...
    def forward(self, x, lambda_value):
        """Forward pass through the MobileNet-V3 model with Gaussian RAMA layers."""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='PyTorch CIFAR-10 Training with MobileNet and Gaussian RAMA Layers')
