...
class Trainer:
    ...
    def evaluate_p(self, lambda_value):
        """
    ...
    def train(self, epochs, start_epoch=0):
        ...
        if self.use_rama and self.use_hyperparameter_optimization:
            if start_epoch == 0:
                ...
                self.best_lambda, _, _ = self.optimize_p()
            else:
        ...
        for epoch in range(start_epoch, epochs):
            ...
            if (self.use_rama and self.use_hyperparameter_optimization and
                ...
                lambda_value, bayesian_score, results = self.optimize_p(
            ...
            if self.neptune_run:
                ...
                if self.use_rama and self.use_hyperparameter_optimization:
                    self.neptune_run["RAMA_P"].append(self.best_lambda)
            ...
            if self.writer:
                ...
                if self.use_rama and self.use_hyperparameter_optimization:
                    self.writer.add_scalar("RAMA_P", self.best_lambda, epoch)
...
def get_experiment_name(args: argparse.Namespace) -> str:
    ...
    if args.use_rama:
        exp_name += f"_{args.lambda_value}" 
    return exp_name
